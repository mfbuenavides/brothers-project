...
<div class="fieldcontain ${hasErrors(bean: accountRoleInstance, field: 'account', 'error')} required">
	...
	<g:select id="account" name="account.id" from="${Account.list()}" optionKey="id" required="" value="${accountRoleInstance?.account?.id}" class="many-to-one"/>
</div>
...
<div class="fieldcontain ${hasErrors(bean: accountRoleInstance, field: 'role', 'error')} required">
	...
	<g:select id="role" name="role.id" from="${Role.list()}" optionKey="id" required="" value="${accountRoleInstance?.role?.id}" class="many-to-one"/>
</div>
